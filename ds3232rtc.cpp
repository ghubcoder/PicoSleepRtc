...
/*
  ...
  Modified by ghubcoder July 2021 to work with Raspberry Pico
...
void write_bytes(uint8_t reg, uint8_t *buf, int len) {
    ...
    i2c_write_blocking(i2c,DS3231_I2C_ADDR, buffer, len + 1, false);
};
...
void DS3231_set(struct ts t)
{
    ...
    for (i = 0; i <= 6; i++) {
        ...
        if (i == 5)
            TimeDate[5] += century;
        i2c_write_blocking(i2c,DS3231_I2C_ADDR,&TimeDate[i],2,false);
    }
}
...
void DS3231_get(struct ts *t)
{
    ...
    i2c_write_blocking(i2c,DS3231_I2C_ADDR, DS3231_TIME_CAL_ADDR,1,true);

    i2c_read_blocking(i2c,DS3231_I2C_ADDR,TimeDate,7,false);

    for (i = 0; i <= 6; i++) {
        if (i == 5) {
            century = (TimeDate[5] & 0x80) >> 7;
            TimeDate[5] = bcdtodec(TimeDate[5] & 0x1F);
        } else
...
{
    //i2c_write_blocking(i2c,DS3231_I2C_ADDR,&addr,1,false);
    //i2c_write_blocking(i2c,DS3231_I2C_ADDR,&val,1,false);
...
uint8_t DS3231_get_addr(const uint8_t addr)
{
    ...
    i2c_write_blocking(i2c,DS3231_I2C_ADDR,&addr,1,true);

    i2c_read_blocking(i2c,DS3231_I2C_ADDR,&rv,1,false);
...
void DS3231_set_a1(const uint8_t s, const uint8_t mi, const uint8_t h, const uint8_t d, const uint8_t * flags)
{
    ...
    uint8_t send_t[4] = { s, mi, h, d };
    uint8_t i;

    uint8_t writeVal;

    const uint8_t reg = DS3231_ALARM1_ADDR;

    for (i = 0; i <= 3; i++) {
        if (i == 3) {
            send_t[i] = dectobcd(t[3]) | (flags[3] << 7) | (flags[4] << 6);
        } else{
            send_t[i] = dectobcd(t[i]) | (flags[i] << 7);
        }
    }

    write_bytes(reg, send_t, 4);
}
...
void DS3231_get_a1(char *buf, const uint8_t len)
{
    ...
    const uint8_t reg= DS3231_ALARM1_ADDR;
    i2c_write_blocking(i2c,DS3231_I2C_ADDR, &reg,1,true);

    i2c_read_blocking(i2c,DS3231_I2C_ADDR,n,4,false);
...
void DS3231_set_a2(const uint8_t mi, const uint8_t h, const uint8_t d, const uint8_t * flags)
{
    ...
    const uint8_t reg = DS3231_ALARM2_ADDR;

    for (i = 0; i <= 2; i++) {
        if (i == 2) {
            send_t[i] = dectobcd(t[2]) | (flags[2] << 7) | (flags[3] << 6);
        } else
            send_t[i] = dectobcd(t[i]) | (flags[i] << 7);
    }
...
void DS3231_get_a2(char *buf, const uint8_t len)
{
    ...
    uint8_t f[4];               // flags
    uint8_t i;
    uint8_t reg=DS3231_ALARM2_ADDR;

    i2c_write_blocking(i2c,DS3231_I2C_ADDR, &reg,1,true);

    i2c_read_blocking(i2c,DS3231_I2C_ADDR,n,3,false);
